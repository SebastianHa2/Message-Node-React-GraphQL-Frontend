    ...
    const graphqlQuery = {
      query: `
        query fetchPosts($page: Int!) {
          getPosts(page: $page) { posts { _id title content imageUrl creator { name } createdAt } total }
        }
      `,
      variables: {
        page: page
    ...
    fetch('http://localhost:8080/graphql', {
      ...
      .then(resData => {
        ...
        this.setState({
          ...
          totalPosts: resData.data.getPosts.total,
          postsLoading: false
        });
      })
      .catch(this.catchError);
    ...
    .then(res => {
      ...
      .then(resData => {
        if (resData.errors && resData.errors[0].status == 422) {
          throw new Error('Invalid input, creating or editing a post failed...');
        }
        if(resData.errors){
          throw new Error('Something went wrong while creating or editing a post, user may not be authenticated...')
        }
